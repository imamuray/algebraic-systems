  ...
  \section{第3章 環と多項式}
    ...
    \begin{frame}
      ...
      \begin{thm}
        ...
        $R \subset F : \text{subring} \Rightarrow \Frac{R} \cong K$
      \end{thm}
    ...
    \begin{frame}
      ...
      \begin{defn}
        $a_0, \cdots, a_m \in \R, x \in \R : \text{variable}$ \\
        $x \mapsto a_0 + a_1 x + \cdots + a_m x^m$ : \alert{多項式写像}または\alert{多項式関数(polynomial function)}
      \end{defn}

      \begin{defn}
        $R$ : commutative ring, $R \neq 0$, $\tilde{P} \coloneqq \{f \mid f\colon \N \to R\}$ \\
        $f \in \tilde{P}, f\{n\} = a_n, f = (a_0, a_1, a_2, \cdots) = (a_n)_{n \ge 0} = (a_n)$
      \end{defn}
      \underline{Rem.} 後に用いる$f(x)$などの表記と区別するため$f\{n\}$とかく.
